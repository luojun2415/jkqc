...
class AdminController extends Controller
{
    ...
    protected function _initialize()
    {
        ...
        $this->assign('__MENUEX__', $this->getMenusEX());
        $this->assign('__MODULE_MENU__', $this->getModules());
        
        //默认项目
        $map['status']    =   array('gt', 0);
        if(!$_GET['gid'] && $_SESSION['gid'])
        	$_GET['gid']=$_SESSION['gid'];
        //echo $_SESSION['gid'];
        if(!IS_ROOT){
        	//if($_GET['gid'])
            $map['id']    =   array('in', get_my_projects($_GET['gid']));
        }
        
        $list = M('jk_project')->where($map)->order('convert(name using gbk) asc')->select();
		//给项目数组加上城市公司
        foreach ($list as &$vvv){
        	//获取项目所属用户组ID的上级ID
        	$city_id = M('auth_group')->where('id='.$vvv['pid'])->getField('pid');
        	$vvv['city_name'] = M('auth_group')->where('id='.$city_id)->getField('title');
        }
        
        if($list && $_GET['gid']){
            //             $list[0]['default']=1;
            //             $_SESSION['proId']=$list[0]['id'];
            //             $this->assign('thisPro', $list[0]['name']);
        	$_SESSION['gid'] = $_GET['gid'];
            $_SESSION['proId'] = $list[0]['id'];
            action_log('show_project', 'JkProjcet', $list[0]['id'], UID);
        }
       // echo 'Admin:'.$_SESSION['proId'];
        //dump(M('jk_project')->_sql());
		// dump(get_my_projects($_GET['gid']));
        $actionid=M('Action')->where("name='show_project' AND module='JkProject'")->getField('id');
        if($actionid){
            $projcetid=M('ActionLog')->where("action_id=$actionid AND user_id=".UID)->order('id DESC')->getField('record_id');
        
            if($projcetid){
                foreach ($list as $k=>$v){
                   // if($v['id']==$projcetid ){之前是用日志判断默认项目，改为通过session
                	if( $v['id']==$_SESSION['proId']){
                        $list[$k]['default']=1;
                        //$_SESSION['proId']=$projcetid;
                        //获取项目所属上级用户组
                        $thisPro=getgroup($v['pid'],'');
                        //设置默认项目
                        $this->assign('thisPro', $thisPro);
                    }
                }
  		...
  		if(!$_SESSION['proId']){
  			...
  			//$_SESSION['proId']=$projcetid;
  			//获取项目所属上级用户组
  			$thisPro=getgroup($list[0]['pid'],'');
  			//设置默认项目
  			$this->assign('thisPro', $thisPro);
  		}
  		//
        $this->assign('_list', $list);
        $this->pro_list = $list;//存入变量
        $map  = array('status' => array('gt', 0),'pid'=> array('gt', 0));
        $pids = M('AuthGroup')->where($map)->field('pid')->select();
		$pids = array_column($pids, 'pid');
		$map  = array('status' => array('gt', 0),'id'=> array('in', $pids));
...
//                 }
    ...
    final protected function editRow($model, $data, $where, $msg)
    {
        ...
        if (M($model)->where($where)->save($data) !== false) {
            action_log('editRow', $model, $id, UID);
    ...
    /**
    * 函数用途描述：获取用户所有子菜单
    * @date: 2016年10月31日 下午2:40:39
    * @author: luojun
    * @param: 
    * @return:
    */
    final public function getMenusEx($controller = CONTROLLER_NAME)
    {
        // $menus  =   session('ADMIN_MENU_LIST'.$controller);
        if (empty($menus)) {
            // 获取主菜单
            ...
            //高亮主菜单
            //$current = M('Menu')->where("url like '%{$controller}/" . ACTION_NAME . "%'")->field('id')->find();
            $current = M('Menu')->where("(url like '{$controller}/" . ACTION_NAME . "%' OR url like '%/{$controller}/"
                 . ACTION_NAME . "%' ) AND pid>0")->getField('id');
		    
            if ($current) {
                $nav = D('menu')->getpath($current);
				
                $nav_url=$controller."/". ACTION_NAME;
                $nav_url= strtolower($nav_url);
// 				dump($nav);
                foreach ($menus['main'] as $key => $item) {
                    if (!is_array($item) || empty($item['title']) || empty($item['url'])) {
                        $this->error(L('_CLASS_CONTROLLER_ERROR_PARAM_',array('menus'=>$menus)));
                    ...
                    //生成child树
                    $groups = M('Menu')->where("pid = {$item['id']} AND hide=0")->distinct(true)->field('group')->order('sort asc')->select();
					$icons = M('Menu')->where("pid = {$item['id']}")->distinct(true)->field('icon')->order('sort asc')->select();
...
// 		dump($menus['child']);
        $open=1;
//          dump($nav);
		foreach($menus['child'] as $k=>$m){
		    
		    if($open){
		        foreach($m as $j=>$v){
		            
		            if($v['id']==$nav[1]['id']&&$nav[1]['id']&&$open){
		                
		                $menus['child'][$k][$j]['open']=1;
		                $menus['child'][$k]['open']=1;
		                $open=0;
		            }
		        } 
		    }
    ...
    final protected function returnNodes($tree = true)
    {
        ...
        if ((int)$tree) {
            $list = M('Menu')->where("hide=0")->field('id,pid,title,url,tip,hide')->order('sort asc')->select();
            ...
            $nodes = M('Menu')->where("hide=0")->field('title,url,tip,pid')->order('sort asc')->select();
            foreach ($nodes as $key => $value) {
                if (stripos($value['url'], MODULE_NAME) !== 0) {
...
//         echo '</pre>';
   ...
   //     echo $p;
        ...
        $all=$model->field($field)->select();
        return $all;
    }
    protected function lists1($model, $where = array(), $order = '', $base = array('status' => array('egt', 0)), $field = true)
    {
        $options = array();
        $REQUEST = (array)I('request.');
        if (is_string($model)) {
        ...
        $options['where'] = array_filter(array_merge((array)$base, /*$REQUEST,*/
            ...
            if (isset($REQUEST['r'])) {
                ...
                $listRows = C('LIST_ROWS') > 0 ? C('LIST_ROWS') : 10000000;
            }
           ...
           // $this->assign('_page', $p ? $p : '');
            ...
            if(!total)
                $total=0;
            echo '<span style="color:gray;margin:5px;">该用户组共有<span style="color:red">'.$total.'</span>个用户</span>';
    ...
    protected function lists2($model, $where = array(), $order = '', $base = array('status' => array('egt', 0)), $field = true)
    {
        $options = array();
        $REQUEST = (array)I('request.');
        if (is_string($model)) {
