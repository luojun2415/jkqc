    ...
    public function upload($files, $setting, $driver = 'local', $config = null){
        ...
        if($info){ //文件上传成功，记录文件信息
            foreach ($info as $key => &$value) {
                ...
                //获取当前项目id
                if($_SESSION['proId1']!="")
                     $value['projectid']=$_SESSION['proId1'];
                else
                     $value['projectid']=$_SESSION['proId'];
                //$value['projectid'] = $_SESSION['proId'];
                $value['create_time'] = time()."000";
                $value['update_time'] = time()."000";
                $value['target_id'] = $value['id'];
                $value['status'] = 1;
