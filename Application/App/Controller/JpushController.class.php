...
class JpushController{
	private $app_key = 'a0701924af63ff8b06a46bf7'; // 待发送的应用程序(appKey)，只能填一个。
	private $master_secret = '398d92cf7404bf8fd9220621'; // 主密码
    private $url = "https://api.jpush.cn/v3/push"; // 推送的地址

//    private $app_key = '751bb2c7866c7d23ebbe9868'; // 待发送的应用程序(appKey)，只能填一个。
//    private $master_secret = '284436a399e8eb0bba79928f'; // 主密码
//
//    private  $url = "https://bjapi.push.jiguang.cn/v3/push";
	 ...
	 * $content ：推送主要内容
	 * $m_type = 推送附加字段的类型,新任务推送，任务过期提醒，任务更改提醒<这么对整改单位已经整改完成提交>
	 * $m_time = "600"; //离线保留时间 
	 */
	// 调用推送方法
	public function send_pub($receives, $content, $m_type='', $m_txt='') {
		header("Content-type:text/html;charset=utf-8");
		$m_time = "86400"; // 离线保留时间
		$receive = array("registration_id"=>$receives);		
		$message = ""; // 存储推送状态
        ...
        file_put_contents('push.log', time().'=================>'.$message, FILE_APPEND);
		//echo $message;/* "<script>alert('推送信息:{$message}')</script>"; */

        return array('code'=>$error_code,'message'=>$message);
	}
