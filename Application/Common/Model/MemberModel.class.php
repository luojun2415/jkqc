...
class MemberModel extends Model
{
    ...
    public function registerMember($nickname = '',$nickpid)
    {
        /* 在当前应用中注册用户 */
        if ($user = $this->create(array('nickname' => $nickname, 'status' => 1, 'pid'=>$nickpid))) {
            $isadd=$this->where("nickname=$nickname")->find();
            $sql=M()->getLastSql();
    ...
    public function initFollow($uid = 0)
    {
        if ($uid != 0) {
