 ...
 * @luoj
 */
class JKProjectController extends AdminController
{

    protected $shopModel;
    protected $shop_configModel;
    protected $shop_categoryModel;
    protected $shop_surveycategoryModel;
    protected $shop_ACcategoryModel;
    ...
    function _initialize()
    {
        ...
        $this->shop_surveycategoryModel = D('JKProject/JKProjectSurvey');
        $this->shop_ACcategoryModel = D('JKProject/JKProjectAC');
        parent::_initialize();
    }
...
// 			echo '退款遇到问题!'.$result['err_code_des'];
	...
	/**
	 * 函数用途描述：日常巡查列表
	 * @date:2016年10月29日 下午14:53
	 * @author:duanmeihua
	 */
    public function surveyOptionList(){
    	//显示页面
    	$builder = new AdminTreeListBuilder();
    	$attr['class'] = 'btn ajax-post';
    	$attr['target-form'] = 'ids';

    	$tree = $this->shop_surveycategoryModel->getTree(0, 'id,title,sort,pid,status');
    	$builder->title('实测实量选项')
    	->setModel('surveyOption')
    	->buttonNew(U('JKProject/addsurveyOption'))
    	->data($tree)
    	->display();
    }
    ...
    /**
     * 函数用途描述：验收检查项列表
     * @date:2018年6月20日 下午14:53
     * @author:罗俊
     */

    public function ACOptionList(){
        //显示页面
        $builder = new AdminTreeListBuilder();
        $attr['class'] = 'btn ajax-post';
        $attr['target-form'] = 'ids';

        $tree = $this->shop_ACcategoryModel->getTree(0, 'id,title,sort,pid,status');
        $builder->title('分户验收选项')
            ->setModel('ACOption')
            ->buttonNew(U('JKProject/addACOption'))
            ->data($tree)
            ->display();
    }
    ...
    /**
     * 函数用途描述：实测实量选项添加
     * @date:2016-10-29 15:03
     * @author:duanmeihua
     */
    public function addACOption($id = 0, $pid = 0){
        $attr['class'] = 'btn ajax-post';
        $attr['target-form'] = 'ids';
        if (IS_POST) {
            $title=$id?L('_EDIT_'):L('_ADD_');
            if ($this->shop_ACcategoryModel->editData()) {
                $this->success($title.L('_SUCCESS_').L('_PERIOD_'), U('JKProject/ACOptionList'));
            } else {
                $this->error($title.L('_FAIL_').L('_EXCLAMATION_').$this->shop_ACcategoryModel->getError());
            }
        } else {
            $builder = new AdminConfigBuilder();
            $categorys = $this->shop_ACcategoryModel->select();
            $opt = array();
            foreach ($categorys as $category) {
                $opt[$category['id']] = $category['title'];
            }
            if ($id != 0) {
                $category = $this->shop_ACcategoryModel->find($id);
            } else {
                $category = array('pid' => $pid, 'status' => 1);
                $father_category_pid=$this->shop_ACcategoryModel->where(array('id'=>$pid))->getField('pid');
//                if($father_category_pid!=0){
//                    $this->error(L('_ERROR_CATEGORY_HIERARCHY_').L('_EXCLAMATION_'));
//                }
            }
            $builder->title(L('添加选项'))->keyId()->keyText('title', L('_TITLE_'))
                ->keySelect('pid', L('_CATEGORY_FATHER_'), L('_CATEGORY_FATHER_SELECT_'), array('0' => L('_CATEGORY_TOP_')) + $opt)
                ->keyStatus()
                ->keyCreateTime()->keyUpdateTime()
                ->keySelect('option_type', L('验收项类型'), L('选择验收项类型'), array('0' => L('检查'),'1'=>'实测'))
                ->keyText('minqualified', L('合格最大值'),'单位:mm')
//                ->keyText('maxqualified', L('合格最小值'),'单位:mm')
//                ->keyText('mindestroy', L('质量锤最小临界值'),'单位:mm')
//                ->keyText('maxdestroy', L('质量锤最大临界值'),'单位:mm')
                ->keyText('pointlength', L('输入框个数'))
                ->data($category)
                ->buttonSubmit(U('JKProject/addACOption'))->buttonBack()->display();
        }
    }
     ...
     * @author:duanmeihua
     */
	public function addsurveyOption($id = 0, $pid = 0){
		$attr['class'] = 'btn ajax-post';
		$attr['target-form'] = 'ids';
		if (IS_POST) {
			$title=$id?L('_EDIT_'):L('_ADD_');
			if ($this->shop_surveycategoryModel->editData()) {
				$this->success($title.L('_SUCCESS_').L('_PERIOD_'), U('JKProject/surveyOptionList'));
			} else {
				$this->error($title.L('_FAIL_').L('_EXCLAMATION_').$this->shop_surveycategoryModel->getError());
			}
		} else {
			$builder = new AdminConfigBuilder();
			$categorys = $this->shop_surveycategoryModel->select();
			$opt = array();
			foreach ($categorys as $category) {
				$opt[$category['id']] = $category['title'];
			}
			if ($id != 0) {
				$category = $this->shop_surveycategoryModel->find($id);
			} else {
				$category = array('pid' => $pid, 'status' => 1);
				$father_category_pid=$this->shop_surveycategoryModel->where(array('id'=>$pid))->getField('pid');
//				if($father_category_pid!=0){
//					$this->error(L('_ERROR_CATEGORY_HIERARCHY_').L('_EXCLAMATION_'));
//				}
			}
			...
			->keySelect('pid', L('_CATEGORY_FATHER_'), L('_CATEGORY_FATHER_SELECT_'), array('0' => L('_CATEGORY_TOP_')) + $opt)
			->keyStatus()
// 			->keyText('qualified', L('合格标准'),'例：8mm')
// 			->keyText('deviation', L('偏差范围'),'例：3mm')
			->keyText('minqualified', L('合格最小值'),'单位:mm')
			->keyText('maxqualified', L('合格最大值'),'单位:mm')
			->keyText('mindestroy', L('质量锤最小临界值'),'单位:mm')
			->keyText('maxdestroy', L('质量锤最大临界值'),'单位:mm')
			->keyText('pointlength', L('输入框个数'))
			->keyCreateTime()->keyUpdateTime()
			->data($category)
			->buttonSubmit(U('JKProject/addsurveyOption'))->buttonBack()->display();
// 			$builder = new AdminConfigBuilder();
// 			$categorys = $this->shop_surveycategoryModel->select();
// 			$opt = array();
// 			foreach ($categorys as $category) {
...
// 			->keySelect('pid', L('_CATEGORY_FATHER_'), L('_CATEGORY_FATHER_SELECT_'), array('0' => L('_CATEGORY_TOP_')) + $opt)
// 			->keyStatus()
// 			->keyText('qualified', L('合格标准'),'例：[-5,8]mm')
// 			->keyText('deviation', L('整改标准'),'例：＞12mm或＜-7.5mm')
// 			->keyText('submitdata',L('质量锤'),'例：＞25mm或＜-15mm')
// 			->keyCreateTime()->keyUpdateTime()
// 			->data($category)
// 			->buttonSubmit(U('JKProject/addsurveyOption'))->buttonBack()->display();
		}
	 ...
	 * 函数用途描述：日常巡查选项禁用
	 */
	public function setsurveyoptionstatus($ids, $status){
		$builder = new AdminListBuilder();
		$builder->doSetStatus('jk_survey_option', $ids, $status);
	}

    /**
     * 函数用途描述：验收选项禁用
     */
    public function setacoptionstatus($ids, $status){
        $builder = new AdminListBuilder();
        $builder->doSetStatus('jk_acoption', $ids, $status);
    }
