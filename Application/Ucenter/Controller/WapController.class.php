...
class VcardController extends Controller{
	...
	protected function _initialize(){
		...
		$this->token=$this->_get('token');
		if(is_null($this->token))$this->token='eeodbq1422333040';
		...
		if (!$this->wxuser||1){
			$db=D('Wxuser');
			$this->wxuser=$db->where(array('token'=>$this->token))->find();
			// echo($db->getLastsql());
		...
		//var_dump(isset($_SESSION[$session_openid_name]));
		if (!isset($_SESSION[$session_openid_name])||!$_SESSION[$session_openid_name]){
			...
			if (isset($_GET['code']) && isset($_GET['state']) && ($_GET['state']=='oauth')){
				...
				if (!$openid){
					$customeUrl=$this->siteUrl.$_SERVER['REQUEST_URI'];
					$customeUrl=str_replace("code", "cdcd", $customeUrl);
					$customeUrl=str_replace("state", "stst", $customeUrl);
