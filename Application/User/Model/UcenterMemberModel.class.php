...
class UcenterMemberModel extends Model
{
     ...
     */
    public function login($username, $password, $type = 1,$from=1)
    {
        ...
        /* 获取用户数据 */
// 		file_put_contents('refundlog.txt','notify:'.json_encode($this)."\r\n", FILE_APPEND);
        //如果是mdm统一账号
        if($from==2){
            $map['_string'] = "PersonnelID is not null and PersonnelID != ''";
        }else{
            $map['_string'] = "PersonnelID is  null or PersonnelID = ''";
        }
        $user = $this->where($map)->find();
		
// 		file_put_contents('refundlog.txt','notify:'.json_encode($user)."\r\n", FILE_APPEND);
        ...
        if (is_array($user) && $user['status']) {
			...
			file_put_contents("login.txt",json_encode($user));
    ...
    public function ucLogin($username, $password,$from=1)
    {
        ...
        //本地帐号信息
        $tox_user = $this->getLocal($username, $password,$from);
        // 关联表无、UC有、本地无的
    ...
    public function getLocal($username, $password,$from=1)
    {
        $aUsername = $username;
